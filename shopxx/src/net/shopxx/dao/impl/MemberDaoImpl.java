...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	public Member findByReferrerCode(String referrerCode){
		...
		try {
			String jpql = "select members from Member members where members.inviteCode = :referrerCode";
			return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("referrerCode", referrerCode).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
	}
	...
	public Long findMemberCount() {
		try {
		String jpql = "select count(*) from Member members ";
