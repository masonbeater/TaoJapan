...
$().ready(function() {
	...
	var $submit = $("#submit");
	var $pointDeduction = $("#pointDeduction");
	...
	var isInteger = function(obj){
		...
		//return obj.conobj>=0&&obj%1 === 0;
	}
	// 积分
	$point.on('change',function() {
		var amountPayable = "${order.amountPayable}";
		var allowPoint = "${order.member.point}"
		var pointvalue = $point.val();
		var pointScale = "${setting.pointScale}";
		var currencySign = "${setting.currencySign}";
		var currencyUnit = "${setting.currencyUnit}";
		if(isInteger(pointvalue)&&(parseInt(allowPoint)>=parseInt(pointvalue))){
			$pointDeduction.html('-'+currencySign+(pointvalue*pointScale).toFixed(2)+currencyUnit);
		}else{
			$pointDeduction.html('-'+currencySign+parseInt(0).toFixed(2)+currencyUnit);
		}
		calculate();
	});
			...
			<div class="info">
				...
				<form id="orderForm" action="create.jhtml" method="post">
					...
					<div class="span12">
						...
						-->
						<dl class="memo statistic">
							<dt>${message("shop.order.point")}:</dt>
							<dd>
								<input type="text" id="point" name="point" maxlength="200"/>
								<strong id="pointDeduction">-${setting.currencySign}0.00${setting.currencyUnit}</strong>
							</dd>
							
						</dl>
						<dl class="memo">
							<dd>
								<span>${message("shop.order.pointRemain")}:<span id="remainPoint">${order.member.point}</span></span>
							</dd>
