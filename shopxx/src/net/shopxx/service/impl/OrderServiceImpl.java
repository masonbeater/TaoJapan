...
import net.shopxx.util.NumberUtils;
...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements OrderService {
	...
	@Transactional(readOnly = true)
	public Order build(Cart cart, Receiver receiver, PaymentMethod paymentMethod, ShippingMethod shippingMethod, CouponCode couponCode, boolean isInvoice, String invoiceTitle, boolean useBalance, String memo,String point) {
		Assert.notNull(cart);
		...
		if(NumberUtils.isInteger(point)&&Integer.parseInt(point)>0){
			if(order.getMember().getPoint()-Long.valueOf(point)>=0){
				Setting setting = SettingUtils.get();
				String pointScale = setting.getPointScale();
				BigDecimal pointDiscount = new BigDecimal(pointScale).multiply(new BigDecimal(point));
				order.setPointDiscount(pointDiscount);
			}
		}else{
			order.setPointDiscount(new BigDecimal(0));
		}
	...
	public Order create(Cart cart, Receiver receiver, PaymentMethod paymentMethod, ShippingMethod shippingMethod, CouponCode couponCode, boolean isInvoice, String invoiceTitle, boolean useBalance, String memo, Admin operator,String point) {
		Assert.notNull(cart);
		Assert.notNull(cart.getMember());
		Assert.notEmpty(cart.getCartItems());
		...
		Order order = build(cart, receiver, paymentMethod, shippingMethod, couponCode, isInvoice, invoiceTitle, useBalance, memo,point);
		...
		if(NumberUtils.isInteger(point)&&Integer.parseInt(point)>0){
			memberDao.lock(member, LockModeType.PESSIMISTIC_WRITE);
			member.setPoint(member.getPoint()-Long.valueOf(point));
			memberDao.merge(member);
			
			Point p  = new Point();
			p.setPoint(Long.valueOf(point));
			p.setType(Point.Type.out);
			p.setMember(order.getMember());
			p.setSourceOrder(order);
			p.setBizType(BizType.ORDER_COST);
			p.setIntro(p.getOrderPointIntro(BizType.ORDER_COST, order.getSn()));
			p.setTotalPoint(order.getMember().getPoint());
			p.setStatus(Point.Status.valid);
			pointDao.persist(p);
		}
	...
	public void complete(Order order, Admin operator) {
		...
		if (order.getShippingStatus() == ShippingStatus.partialShipment || order.getShippingStatus() == ShippingStatus.shipped) {
			...
			member.setPayPoint(member.getPayPoint()+order.getPoint());
		...
		p.setType(Point.Type.in);
		p.setMember(order.getMember());
		p.setSourceOrder(order);
		p.setBizType(BizType.ORDER_GET);
		p.setIntro(p.getOrderPointIntro(BizType.ORDER_GET, order.getSn()));
		p.setTotalPoint(order.getMember().getPoint());
		p.setStatus(Point.Status.valid);
		pointDao.persist(p);
	}
	...
	public void refunds(Order order, Refunds refunds, Admin operator) {
		...
		if(refunds.getMethod()==Refunds.Method.offline){
